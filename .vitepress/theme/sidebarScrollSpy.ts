// Scroll-spy for left sidebars whose sub-items are in-page hash-anchor links
// (e.g. `/features/#developer-features`), rather than links to separate pages.
//
...
//
//   1. Runs its own scroll-spy (same algorithm as `useActiveAnchor`), scoped
//      to the configured page prefix and heading selector.
...
//
// Configured per call site (see `theme/index.ts`) with a page-path prefix and
// a heading-level selector, since different sections use different heading
// levels for their sidebar sub-items: FAQ sections link to `### ` (h3)
// question headings within collapsible sidebar groups, while Features links
// to `## ` (h2) headings in a flat sidebar.
//
...
export interface SidebarScrollSpyConfig {
  /** Only pages whose relativePath starts with this run the spy (and the "scroll active item into view" step). */
  pathPrefix: string;
  /** CSS selector, scoped under .VPDoc, for this section's heading anchors (e.g. "h3[id]" or "h2[id]"). */
  headingSelector: string;
}

export function useSidebarScrollSpy(config: SidebarScrollSpyConfig): void {
  if (!inBrowser) return;
  ...
  function collectHeaders() {
    headers = [
      ...document.querySelectorAll<HTMLHeadingElement>(`.VPDoc :where(${config.headingSelector})`),
    ]
  ...
  function setActiveHeading() {
    // Only pages under this instance's `config.pathPrefix` have hash-based
    // sidebar sub-items to highlight (FAQ section pages generated by
    // .vitepress/faqSidebar.ts, or the Features page's flat sidebar); every
    // other page — including a section's own overview page, which has no
    // matching headings — has nothing for this to highlight, so skip
    // entirely.
    if (!page.value.relativePath.startsWith(config.pathPrefix)) return;
    ...
    // than force-highlight the first heading — this mirrors VitePress's own
    // right-hand outline (`useActiveAnchor`'s `scrollY < 1` branch also
    // clears the active link). For FAQ this is harmless since the hash-less
    // parent sidebar entry (e.g. "Concepts & Features") stays highlighted
    // via `hasActiveLink` regardless; for Features it correctly leaves
    // nothing highlighted until the reader scrolls to the first heading.
    if (scrollY < 1) {
      applyHash(null);
      return;
    }
  ...
  // after every client-side navigation within this section. `onContentUpdated`
  // guarantees the new page's heading elements are already in the DOM, unlike
  ...
  onContentUpdated(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // when the URL actually points at a specific heading (a hash present)
      // — a plain section-page load has nothing more specific than the
      // page's own nav entry to scroll to, so leave the sidebar alone.
      if (page.value.relativePath.startsWith(config.pathPrefix) && location.hash) {
